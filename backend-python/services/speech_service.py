...
class SpeechService:
    ...
    def __init__(self) -> None:
        ...
        self._api_key = settings.custom_api_key
        self._base_url = settings.custom_base_url
    ...
    async def recognize(self, audio_bytes: bytes) -> str:
        """识别 WAV 音频，返回文本。"""
        data_url = self._encode_data_url(audio_bytes, "audio/wav")
    ...
    async def _call_asr(self, data_url: str) -> str:
        ...
        logger.info("speech.asr.start", model=self._model)
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                logger.info("speech.asr.done", text_length=len(text))
                return text.strip()
        except httpx.HTTPError as e:
            logger.error("speech.asr.error", error=str(e))
            raise RuntimeError(f"语音识别请求失败: {e}") from e
        except (KeyError, IndexError) as e:
            logger.error("speech.asr.parse_error", error=str(e))
